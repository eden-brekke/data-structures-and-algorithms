...
// Solution code here...
  let testArr = [...arr];
  let sortArr = testArr.sort((a,b)=> b.length - a.length);
...
const findHappiness = (arr) => {
  // Solution code here...
  return arr.filter(happyString => happyString.includes(`:)`));
};
...
const onlyOddChars = (str) => {
  // Solution code here...
  let newArr = [];
  str.split('').reduce((element, value, index)=>index % 2 === 1 ? newArr.push(value) : '', []);
  return newArr.join('');
};
