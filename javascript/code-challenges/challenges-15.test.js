...
const detectTicTacToeWin = (board) => {
  // Solution code here...
  let Board = board.flat();
  let columns = [0,1,2].map((e) => [Board[0+e],Board[3+e],Board[6+e]]);
  let diagonals = [[Board[0],Board[4],Board[8]],[Board[2],Board[4],Board[6]]];
  let test = [...columns,...diagonals,...board];
  return test.findIndex(arr => arr.every(position => position === 'X') || arr.every(position => position === 'O')) !== -1;
};
